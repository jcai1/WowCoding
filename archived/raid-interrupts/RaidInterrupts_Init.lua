...
local function deepcopy(orig) -- Will explode if table is recursive
...
-- Sanity check for A.intDB; also make icon names full paths
for class, ints in pairs(A.intDB) do
    for k = 1, #ints do
        ...
        if not (int.spell or int.spellID) then
			...
			int.icon = "Interface\\Icons\\" .. int.icon
        end
    end
end
if A.aborted then
...
function A.updateUnit(unit)
	local u, pet = unit, unit .. "pet"
...
function A.updateDisplay(dispIndex, unit)
	local i, u = dispIndex, unit
	...
	local icon, cd, cdFrame, border = F.icons[i], F.cds[i], F.cdFrames[i], F.borders[i]
	icon:Hide()
	cd:Hide()
	cdFrame:Hide()
	border:Hide()
	...
	local count, charges, minRecharge, iconPath = 0, 0, math.huge, nil
